...
void printString (uint8_t x, uint8_t row, char *str, uint8_t size)
{
    uint8_t *ptr = oled_data;
    uint8_t length = drawString(ptr, str);
    if (size > length) {
        memset(ptr + length, 0, size - length); // Doesn't seem to work.
    }
    ssd1306_setActiveArea(x, x+size-1, row, row);
...
void printVoltage (float voltage) 
{
    ...
    printString(0, 0, str, (sizeof(str)-1)*6);
}
...
void printCurrent (float value)
{
    ...
    printString(0, 1, str, (sizeof(str)-1)*6);
}
...
void printPower (float value)
{
    ...
    printString(0, 2, str, (sizeof(str)-1)*6);
}
...
void printCharge (float value)
{
    ...
    printString(0, 3, str, (sizeof(str)-1)*6);
}